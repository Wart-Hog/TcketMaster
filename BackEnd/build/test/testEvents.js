...
describe('Post Test', function () {
    it('/events [201]', function (done) {
        supertest_1.default(app).post('/events').set("token", "E2h7nrvWLpJJoFya8SpXpG").send({ name: "nomeevento", type: "music", place: "placeevento", dateTime: "1/1/2000", price: "10" }).expect(201, done);
    });
    it('/events [400] errore nella data', function (done) {
        supertest_1.default(app).post('/events').set("token", "E2h7nrvWLpJJoFya8SpXpG").send({ name: "nomeevento", type: "music", place: "placeevento", dateTime: "null", price: "10" }).expect(400, done);
    });
    it('/events [400] errore nel type', function (done) {
        supertest_1.default(app).post('/events').set("token", "E2h7nrvWLpJJoFya8SpXpG").send({ name: "nomeevento", type: "null", place: "placeevento", dateTime: "1/1/2000", price: "10" }).expect(400, done);
    });
...
describe(" Delete Test", function () {
    it('/events [200]', function (done) {
        supertest_1.default(app).delete('/events/4321-4cd9-a969-b89c9969149f').set("token", "E2h7nrvWLpJJoFya8SpXpG").expect(201, done);
    });
    it('/events [401]', function (done) {
        supertest_1.default(app).delete('/events/4321-4cd9-a969-b89c9969149f').set("token", "null").expect(401, done);
    });
});
