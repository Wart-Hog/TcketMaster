...
describe("Get Test", function () {
    ...
    it('/users/:username [200]', function (done) {
        supertest_1.default(app).get('/users/pincpall').set("token", "LGB3G9e2tpq6PTFpj1Ssn5").expect(200, done);
    });
...
describe("POST test", function () {
    ...
    it('/users/:username/tickets [200]', function (done) {
        supertest_1.default(app).post('/users/pincpall/tickets').set("token", "C9ioCXoenoTTvWKyvBDKKD").send({ eventId: "4f6dfac1-31a5-4097-93ab-3d5d8e9bae19" }).expect(200, done);
    });
    it('/users/:username/tickets [401]', function (done) {
...
describe('DELETE test', function () {
    ...
    it('/users [404]', function (done) {
        supertest_1.default(app).delete('/users').set("token", "C9ioCXoenoTTvWKyvBDKKD").send({ username: "mario" }).expect(404, done);
    });
});
