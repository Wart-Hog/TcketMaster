...
describe('Delete test',()=>{
    it('/users [201]',(done)=>{//ok
        request(app).delete('/users').set("token","Xo7MsuSZN91ET7wPoLZ8eJ").send({username:"pippo"}).expect(201,done)
    });
    it('/users [401]',(done)=>{//ok
        request(app).delete('/users').set("token","null").send({username:"pippo"}).expect(401,done)
    });
    it('/users [404]',(done)=>{//ok
        request(app).delete('/users').set("token","E2h7nrvWLpJJoFya8SpXpG").send({username:"mario"}).expect(404,done)
    });
});
